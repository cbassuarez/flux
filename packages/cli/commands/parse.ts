...
export async function runParseCommand(argv: string[]): Promise<void> {
  const { flags, positional } = parseArgs(argv);

  const options: ParseOptions = {
    outputPath: (flags.o as string) ?? (flags.output as string) ?? undefined,
  ...
  if (options.stdin) {
    ...
    try {
      ...
      hadError = true;
      printParseError("<stdin>", err as Error);
    }
  } else {
    ...
    for (const file of positional) {
      ...
      try {
        ...
        hadError = true;
        console.error(`flux parse: failed to read ${file}: ${(err as Error).message}`);
        continue;
      }
      ...
      try {
        ...
        hadError = true;
        printParseError(file, err as Error);
      }
    }
  }
...
function printParseError(file: string, err: Error): void {
  // Your parser errors already include line/column info in the message string.
  console.error(`flux parse: ${file}: ${err.message}`);
}

function formatOutput(files: ParsedFile[], opts: ParseOptions): string {
  if (opts.summary) {
    return files.map((f) => summarizeDoc(f.file, f.doc as any)).join("\n\n");
  }
...
function summarizeDoc(file: string, doc: any): string {

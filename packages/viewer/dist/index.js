            ...
            if (url.pathname === "/api/edit/transform" && req.method === "POST") {
                ...
                try {
                    ...
                    if (op === "setTextNodeContent") {
                        ...
                        const result = applyMoveNodeTransform(source, parsed.doc, { nodeId, fromContainerId, toContainerId, toIndex }, docPath, parseSource);
                        if (!result.ok) {
                            sendTransform({ ok: false, diagnostics: buildDiagnosticsBundle([result.diagnostic]) }, { applied: false });
                            return;
                        }
...
function applyInsertPageTransform(source, doc, { afterPageId }, docPath) {
...
function parseContainerId(raw) {
    if (raw.startsWith("page:"))
        return { kind: "page", id: raw.slice("page:".length) };
    if (raw.startsWith("section:"))
        return { kind: "section", id: raw.slice("section:".length) };
    return raw ? { kind: "section", id: raw } : null;
}
function resolveSectionContainer(source, doc, container, docPath, createIfMissing, parseSource) {
    ...
    const ids = collectIdsFromDoc(doc);
    const sectionId = nextId("section", ids);
    const nextSection = { id: sectionId, kind: "section", props: {}, children: [] };
    const nextPage = { ...page, children: [...(page.children ?? []), nextSection] };
...
function applyMoveNodeTransform(source, doc, { nodeId, fromContainerId, toContainerId, toIndex, }, docPath, parseSource) {
    if (!nodeId) {
        return { ok: false, diagnostic: buildDiagnosticFromMessage("Missing node id", source, docPath, "fail") };
    }
    ...
    if ("diagnostic" in resolvedSource)
        return { ok: false, diagnostic: resolvedSource.diagnostic };
    const resolvedTarget = resolveSectionContainer(resolvedSource.source, resolvedSource.doc, toRef, docPath, true, parseSource);
    if ("diagnostic" in resolvedTarget)
        return { ok: false, diagnostic: resolvedTarget.diagnostic };
    ...
    if (sourceSection.id === targetSection.id) {
        ...
        const nextSection = { ...sourceSection, children };
    ...
    const nextSourceSection = { ...sourceSection, children: sourceChildren };
    const removeResult = applyReplaceNodeTransform(resolvedTarget.source, resolvedTarget.doc, nextSourceSection.id, nextSourceSection, docPath);
    if (!removeResult.ok) {
    ...
    const nextTargetSection = { ...refreshedTarget, children: targetChildren };
...
function applyReplaceNodeTransform(source, doc, id, replacement, docPath) {
    ...
    if (!replacement || replacement.id !== id) {
        return {
            ok: false,
            diagnostic: buildDiagnosticFromMessage(`Replacement node id does not match '${id}'.`, source, docPath, "fail"),
        };
    }
...
const PRINT_INDENT = "  ";
function printDocumentNode(node, indent) {
    const lines = [];
...
function printNodePropValue(value) {

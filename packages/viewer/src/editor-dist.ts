...
export async function resolveEditorDist(options: ResolveEditorDistOptions = {}): Promise<EditorDistResolution> {
  const env = options.env ?? process.env;
  const fsImpl = options.fsImpl ?? fs;
  const tried: string[] = [];

  const maybeCheck = async (rawPath: string | undefined, source: EditorDistSource): Promise<EditorDistResolution | null> => {
...
async function hasIndex(fsImpl: Pick<typeof fs, "stat" | "access">, dir: string): Promise<boolean> {
  try {
...
export function buildEditorMissingHtml(resolution: EditorDistResolution): string {
  ...
  return [
    ...
    reason,
    "<p>Build the in-repo editor bundle (base <code>/edit/</code>) and sync it into the viewer package:</p>",
    "<pre><code>npm run build --workspace @flux-lang/editor-ui</code></pre>",
    "<pre><code>npm run sync-editor</code></pre>",
    "<p>Or point the viewer at a local editor build:</p>",
    "<pre><code>FLUX_EDITOR_DIST=/abs/path/to/editor/dist flux view &lt;file&gt;</code></pre>",
    "<p>CLI flag option:</p>",

// Minimal in-memory runtime for Flux documents

// A simple deterministic PRNG (xorshift32)
...
function evalCall(expr, ctx) {
  ...
  if (expr.callee.kind === "MemberExpression" && expr.callee.property === "contains") {
...
function resolveAssignmentTarget(target) {
      ...
      for (let row = 0; row < grid.rows; row++) {
        for (let col = 0; col < grid.cols; col++) {
          ...
          for (const rule of gridRules) {
            const ctx = { state, grid, cell, row, col };
            ...
            if (condition) {
              ...
              for (const stmt of assignments) {
                ...
                const target = resolveAssignmentTarget(stmt.target);
                if (!target) continue;

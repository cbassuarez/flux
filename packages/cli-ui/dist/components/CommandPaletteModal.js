...
export function CommandPaletteModal({ query, groups, selectedId, width, debug, }) {
    return (_jsx(Box, { width: width, children: _jsxs(Card, { title: "Command Palette", meta: "Ctrl+K", accent: true, ruleWidth: width - 6, debug: debug, children: [_jsxs(Box, { flexDirection: "row", gap: 1, children: [_jsx(Text, { color: color.muted, children: "Search" }), _jsx(InputLine, { value: query, placeholder: "Type to filter", focused: true })] }), _jsx(Box, { flexDirection: "column", gap: 1, children: groups.length === 0 ? (_jsx(Text, { color: color.muted, children: "No matches" })) : (groups.map((group) => (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: color.muted, children: group.group }), group.items.map((item) => {

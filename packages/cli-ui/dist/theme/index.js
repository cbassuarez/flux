...
export const isTruecolor = (() => {
    ...
    if (TRUECOLOR_ENV.some((entry) => env.includes(entry)))
        return true;
...
const mutedRule = (width) => "-".repeat(Math.max(0, width));
...
export function accentRule(width) {
...
export function mutedRuleText(width) {
    const safeWidth = Math.max(0, width);
    if (safeWidth === 0)
        return "";
    if (!isTruecolor)
        return chalk.gray(mutedRule(safeWidth));

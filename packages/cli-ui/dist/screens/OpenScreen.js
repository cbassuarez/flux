...
export function OpenScreen({ width, query, showAll, rootDir, results, selectedIndex, folders, folderIndex, activeList, pinnedDirs, recentDirs, isPinned, indexing, truncated, preview, onToggleShowAll, onOpenSelected, onSelectResult, onSelectFolder, onSelectPinned, onSelectRecent, onTogglePin, debug, }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", gap: 1, children: [truncated ? (_jsx(Card, { title: "Index limit reached", meta: "", ruleWidth: width - 6, debug: debug, children: _jsx(Text, { color: color.muted, children: "Too many files; narrow search or browse folders." }) })) : null, _jsxs(Card, { title: "Open", meta: indexing ? "indexing…" : `${results.length} results`, accent: true, ruleWidth: width - 6, debug: debug, children: [_jsxs(Box, { flexDirection: "row", gap: 1, alignItems: "center", children: [_jsx(Text, { color: color.muted, children: "Search" }), _jsx(InputLine, { value: query, placeholder: "Type to filter" }), _jsx(Clickable, { id: "toggle-filter", onClick: onToggleShowAll, priority: 1, children: _jsx(Text, { color: color.muted, children: showAll ? "Filter: all" : "Filter: *.flux" }) })] }), _jsxs(Box, { flexDirection: "column", gap: 0, children: [_jsx(Text, { color: color.muted, children: activeList === "results" ? "Results" : "Results (inactive)" }), results.length === 0 ? (_jsx(Text, { color: color.muted, children: "No matches yet." })) : (_jsxs(Box, { flexDirection: "column", gap: 0, children: [resultsList.showTop ? _jsx(Text, { color: color.muted, children: "\u2026" }) : null, resultsList.items.map((item, idx) => {

...
function readFluxVersionFromEnv() {
    if (typeof process === "undefined")
        return undefined;
    return process.env?.FLUX_VERSION;
}
function formatFluxBadgeVersion(rawVersion) {
    const raw = (rawVersion ?? "").toString().trim();
    if (raw.length === 0)
        return FLUX_BADGE_FALLBACK_VERSION;
    return raw.startsWith("v") ? raw : `v${raw}`;
}
function resolveThemeClasses(theme) {
    if (theme === "dark") {
...
export function FluxBadge({ className, theme, version }) {
    ...
    return (_jsxs("a", { href: FLUX_BADGE_HREF, target: "_blank", rel: "noreferrer", className: mergedClassName, "aria-label": `${FLUX_BADGE_PACKAGE} package (version ${versionLabel})`, children: [_jsx("img", { src: FLUX_BADGE_MARK_SRC, alt: "Flux mark", className: "h-4 w-auto" }), _jsx("span", { className: "uppercase tracking-wide", children: FLUX_BADGE_PACKAGE }), _jsx("span", { className: "text-[11px] text-slate-500", children: versionLabel })] }));
}
//# sourceMappingURL=flux-badge.js.map
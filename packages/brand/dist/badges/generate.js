...
async function parsePnpmWorkspacePatterns(repoRoot) {
...
async function readBadgeConfig(repoRoot) {
    ...
    if (existsSync(rootConfigPath)) {
        const root = await readJson(rootConfigPath);
        config = { ...config, ...root };
    }
    return config;
}
function inferRepoFromRemote(remoteUrl) {
    ...
    if (!match)
        return { org: "cbassuarez", repo: "flux" };
...
async function resolveWorkflowFile(repoRoot, config, workflowFiles) {
    const workflowDir = path.join(repoRoot, ".github", "workflows");
...
async function expandWorkspacePatterns(repoRoot, patterns) {
        ...
        if (pattern.endsWith("/*")) {
            ...
            for (const entry of entries) {
                if (entry.isDirectory())
                    directories.add(path.join(baseDir, entry.name));
            }
            continue;
        }
...
async function collectWorkspacePackages(repoRoot, config) {
...
async function detectLicense(repoRoot) {
    const rootPackage = await readJson(path.join(repoRoot, "package.json"));
    if (typeof rootPackage.license === "string" && rootPackage.license.trim())
        return rootPackage.license.trim();
    const candidatePaths = ["LICENSE", "LICENSE.md", "LICENSE.txt"].map((name) => path.join(repoRoot, name));
    for (const candidate of candidatePaths) {
        if (!existsSync(candidate))
            continue;
        const raw = await readFile(candidate, "utf8");
        if (/MIT License/i.test(raw))
            return "MIT";
        if (/Apache License/i.test(raw))
            return "Apache-2.0";
        if (/GNU GENERAL PUBLIC LICENSE/i.test(raw))
            return "GPL";
        return "custom";
    }
...
async function resolveCiStatus(metadata) {
    const token = process.env.GITHUB_TOKEN || process.env.GH_TOKEN;
    if (!token || !metadata.workflowFile)
        return "unknown";
...
async function writeThemePair(outputDir, fileBase, render) {
...
function relativeAssetPath(repoRoot, readmePath, fileBase, theme) {
...
function renderPictureBadge(repoRoot, readmePath, spec) {
...
function buildBadgeBlock(repoRoot, readmePath, specs) {
...
function removeLegacyBadgeSection(lines, insertAt) {
    if (insertAt >= lines.length)
        return insertAt;
    if (lines[insertAt].trim() === "<p>") {
        let closingIndex = insertAt + 1;
        while (closingIndex < lines.length && lines[closingIndex].trim() !== "</p>")
            closingIndex += 1;
...
function upsertBadgeBlock(markdown, block) {
...
function buildRootBadgeSpecs(context) {
...
function buildPackageBadgeSpecs(context) {
...
function resolveLicenseHref(repoRoot, metadata, readmePath) {
...
function normalizeVersionLabel(rawVersion, fallback = "v0.0.0-dev") {
    const formatted = formatBadgeVersion(rawVersion);
    return formatted ?? fallback;
}
async function readFluxMarkDataUri(repoRoot) {
...
async function ensureReadmeExists(readmePath, packageName) {
    if (existsSync(readmePath))
        return;
    await writeFile(readmePath, `# ${packageName}\n`, "utf8");
}
async function resolveRepoMetadata(repoRoot, config) {
...
export async function generateBadges(repoRoot = process.cwd()) {
    ...
    const explicitPackages = Array.isArray(config.npmPackages)
        ? config.npmPackages.filter((value) => typeof value === "string" && value.length > 0)
        : [];
    ...
    for (const pkg of workspacePackages) {
        if (pkg.version)
            packageVersionByName.set(pkg.name, pkg.version);
    }
    const npmInfos = await Promise.all(packageNames.map((name) => fetchNpmInfo(name)));
    const npmInfoByName = new Map(npmInfos.map((info) => [info.packageName, info]));
    ...
    for (const packageName of packageNames) {
        ...
        if (packageName !== FLUX_PACKAGE_NAME) {
            await writeThemePair(outputDir, `npm.${slug}`, (theme) => renderBadgeSvg({
                kind: "npm",
                label: packageName,
        ...
        await writeThemePair(outputDir, `channel.${slug}.stable`, (theme) => renderBadgeSvg({
            kind: "channel",
            label: "stable",
            value: stableVersionLabel,
            theme,
        ...
        if (typeof canaryRaw === "string" && canaryRaw.trim()) {
            ...
            await writeThemePair(outputDir, `channel.${slug}.canary`, (theme) => renderBadgeSvg({
                kind: "channel",
                label: "canary",
    ...
    await writeThemePair(outputDir, "ci", (theme) => renderBadgeSvg({
        kind: "ci",
        label: "CI",
        value: ciStatus,
        theme,
    ...
    await writeThemePair(outputDir, "license", (theme) => renderBadgeSvg({
        kind: "license",
        label: "License",
        value: license,
        theme,
    ...
    await writeThemePair(outputDir, "docs", (theme) => renderBadgeSvg({
        kind: "docs",
        label: "Docs",
        value: "site",
        theme,
    ...
    await writeThemePair(outputDir, "discord", (theme) => renderBadgeSvg({
        kind: "discord",
        label: "Community",
        value: "chat",
        theme,
    ...
    await writeThemePair(outputDir, "security", (theme) => renderBadgeSvg({
        kind: "security",
        label: "Security",
        value: "policy",
        theme,
    ...
    await writeThemePair(outputDir, "maintained", (theme) => renderBadgeSvg({
        kind: "maintained",
        label: "Maintained",
        value: maintained,
        theme,
    ...
    await writeThemePair(outputDir, "flux-npm", (theme) => renderFluxNpmBadgeSvg({
    ...
    if (existsSync(rootReadmePath)) {
        ...
        const rootBlock = buildBadgeBlock(repoRoot, rootReadmePath, buildRootBadgeSpecs({
            repoRoot,
            metadata,
    ...
    for (const pkg of workspacePackages) {
        ...
        const stableVersionLabel = normalizeVersionLabel(stableRaw, BADGE_FALLBACK_VALUE);
        const canaryVersionLabel = typeof info.distTags.canary === "string" && info.distTags.canary.trim()
        ...
        const packageBlock = buildBadgeBlock(repoRoot, pkg.readmePath, buildPackageBadgeSpecs({
            repoRoot,
            metadata,
            licenseHref,
...
export async function runBadgeGeneratorCli() {
